...
import math
# reference:https://github.com/romulus0914/MixNet-PyTorch
...
class MDConv(nn.Module):
    def __init__(self, channels, kernel_size, stride):
        ...
        #需要对序列进行操作时可以这么声明，即在forward进行序列化操作，类似与列表（list)
        ...
        for i in range(self.num_groups):
            self.mixed_depthwise_conv.append(nn.Conv2d(
                ...
                kernel_size[i],
                stride=stride,
                padding=kernel_size[i] // 2,#此方法能够保证计算后的特征图长宽不变
                groups=self.split_channels[i],#深度可分离卷积
                bias=False
            ))
...
class MixNetBlock(nn.Module):
    def __init__(
        ...
        # expand_ksize的个数并作为kernelsize进行卷积，对维度进行分组，分别对每组进行维度扩充，然后再在维度进行拼接
        if expand:
            # expansion phase
        ...
        #将通道进行均分，一组通道对应一个kernelsize,最后将每组结果在通道上拼接
        dw = nn.Sequential(
        ...
        if se:
            # squeeze and excite
            # 对通道进行缩放，然后再进行扩张最后输出in_channels*se_retio
...
class MixNet(nn.Module):
    ...
    def __init__(self, net_type='mixnet_s', input_size=224, num_classes=1000, stem_channels=16, feature_size=350,
        ...
        self.avgpool = nn.AvgPool2d((input_size[0] // 4,input_size[1] // 4), stride=1)
        self.dropout = nn.Dropout(dropout_rate)
        self.classifier = nn.Linear(feature_size, num_classes)
        self.softmax = nn.Softmax(dim=1)
#2021年3月10日11:19:06 增加softmax
...
if __name__ == '__main__':
    net = MixNet(input_size=48,num_classes=2)
    # print(net)
    x_image = Variable(torch.randn(1, 3, 48, 48))
    x = torch.rand(1,3,48,48).float()
    y = net(x_image)
    torch.save(net.state_dict(),"mixnet_change.pth")
    torch_out = torch.onnx.export(net,x,"mxinet_change_onnx.onnx",verbose=True,input_names=["input"],output_names=["out_put"])
    print("dfew")