...
from mtcnn.core.models import PNet, ONet, RNet_5_old
...
def create_mtcnn_net(p_model_path=None, r_model_path=None, o_model_path=None, use_cuda=True):
    ...
    if r_model_path is not None:
        rnet = RNet_5_old(use_cuda=use_cuda)
        if (use_cuda):
    ...
    if o_model_path is not None:
        # onet = ONet_old(use_cuda=use_cuda)
...
class MtcnnDetector(object):
    ...
    def detect_pnet(self, im):
        ...
        while min(current_height, current_width) > net_size:
            ...
            keep = utils.nms(boxes[:, :5], 0.5, 'Union')  # 第一次NMS，可优化的地方
            boxes = boxes[keep]
            # print(boxes.shape)
    ...
    def detect_onet(self, im, dets):
        ...
        wearmask = None
        try:
            cls_map, reg, landmark, wearmask = self.onet_detector(feed_imgs)
        except Exception:
        ...
        reg = reg.cpu().data.numpy()
        landmark = landmark.cpu().data.numpy()
        if wearmask is not None:
            wearmask = wearmask.cpu().data.numpy()
        ...
        land_mark_list = []
        for j in range(keep_landmark.shape[1]):
            temp = (keep_landmark[:, j] * bw + align_landmark_topx) if j % 2 == 0 else (
                    keep_landmark[:, j] * bh + align_landmark_topy)
            land_mark_list.append(temp)

        landmark = np.array(land_mark_list)
        landmark_align = landmark.T

        return boxes_align, landmark_align, keep_wearmask

    def detect_face(self, img, rgb=False):
        ...
        if self.pnet_detector:
            ...
            if boxes_align is None:
                return np.array([]), np.array([]), wearmask_pred
        ...
        if self.onet_detector:
            boxes_align, landmark_align, wearmask_pred = self.detect_onet(img, boxes_align)
            if boxes_align is None:
                return np.array([]), np.array([]), wearmask_pred
        ...
        return boxes_align, landmark_align, wearmask_pred
